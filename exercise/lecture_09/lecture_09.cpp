...
template <typename C, typename val = typename C::value_type, typename = std::enable_if_t< std::is_same_v<val,int>>>
int sum_single(const C& container)
{
    return std::accumulate(container.begin(), container.end(), 0);
}

template <typename C, typename val = typename C::value_type, typename = std::enable_if_t< std::is_same_v<val,int>>>
int sum(const C& c1, const C& c2)
{
    return std::accumulate(c1.begin(), c1.end(), 0) + std::accumulate(c2.begin(), c2.end(), 0);
}

template <typename... Cs>
int sum(const Cs&... containers)
{
    return (0 + ... + sum_single(containers));
}
